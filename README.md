...
- Projection page (per portfolio): project a stock's revenue, net income, EPS, and share price with editable revenue-growth, net-margin, and PE-low/high assumptions; supports multiple named scenario sets, persists assumptions (including a manual currency for offline use) and point-in-time snapshots per symbol, and falls back to manual entry when the market-data proxy is unavailable; scenario maths, not advice
